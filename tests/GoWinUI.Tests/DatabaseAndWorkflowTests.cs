...
public sealed class DatabaseAndWorkflowTests
{
    ...
    [Fact]
    public async Task CodingModePersistsAcrossDatabaseReopenAndKeepsExistingSessionData()
    {
        await using var environment = await TestEnvironment.CreateAsync();
        var chats = environment.Get<IChatRepository>();
        var session = await chats.CreateSessionAsync("Coding-Projekt");
        await chats.SetPinnedAsync(session.Id, true);
        var turn = await chats.AddTurnAsync(session.Id, "Passe das Projekt an.");
        await chats.SetPersistentToolActionAsync(session.Id, PersistentToolAction.Coding);
        var revision = (await chats.GetSessionAsync(session.Id))?.ConversationRevision;
        ...
        await reopened.InitializeAsync();
        var repository = new GoWinUI.Infrastructure.Repositories.SqliteChatRepository(reopened);
        var restored = Assert.IsType<ChatSession>(await repository.GetSessionAsync(session.Id));
        Assert.Equal(PersistentToolAction.Coding, restored.PersistentToolAction);
        Assert.Equal(PersistentToolAction.Coding, Assert.Single(await repository.ListSessionsAsync()).PersistentToolAction);
        Assert.Equal(session.Title, restored.Title);
        Assert.True(restored.IsPinned);
        Assert.Equal(revision, restored.ConversationRevision);
        Assert.Equal(
        ...
        await using var connection = new SqliteConnection($"Data Source={reopened.DatabasePath}");
        await connection.OpenAsync();
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT persistent_tool_action FROM chat_sessions WHERE id=$id;";
    ...
    public async Task CurrentSchemaPersistsAudiobookStateAndRemovesLegacySpeechCache()
    {
        ...
        Assert.True(SqliteDatabase.CurrentSchemaVersion >= 30);
        await using (var connection = new SqliteConnection($"Data Source={environment.Get<IGoDatabase>().DatabasePath}"))
        {
            await connection.OpenAsync();
            await using var command = connection.CreateCommand();
