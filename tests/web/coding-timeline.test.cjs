...
test("Blender stages show saved revisions and distinguish visible previews from protected user edits", () => {
  const { context, render } = harness({ codingToolStepsExpanded: true });
  const input = { operation: "stage", path: "steps/02-habitat.py", label: "Habitat-Fenster", baseScene: "01.blend", outputPath: "02.blend" };
  assert.equal(context.codingToolLabel("blender.execute", JSON.stringify(input)), "3D-Etappe modellieren");
  for (const state of ["ready", "blocked"]) {
    const timeline = render(answer(), [step({ tool: "blender.execute", inputJson: JSON.stringify(input), outputJson: JSON.stringify({
      success: true, operation: "stage", outputPath: "02.blend", preview: { state, message: state === "ready" ? "Habitat-Fenster" : "Bitte eigene Änderungen speichern." }
    }) })]);
    assert.ok(timeline.textContent.includes("02.blend"));
    assert.ok(timeline.textContent.includes("Habitat-Fenster"));
    assert.ok(timeline.textContent.includes(state === "ready" ? "In Blender angezeigt" : "Vorschau schützt eigene Änderungen"));
    if (state === "blocked") assert.ok(!timeline.textContent.includes("Im Blender-Fenster angezeigt"));
  }
});

test("Blender receipts show their actual operation label and retain readable legacy fallbacks", () => {
  const { context, render } = harness();
  const labels = {
    info: "Blender und Szenenstand prüfen", scaffold: "3D-Projekt vorbereiten",
    run: "3D-Modell aufbauen oder ändern", inspect: "3D-Geometrie prüfen",
    render: "3D-Ansichten rendern", open: "Blender-Szene öffnen"
  };
  for (const [operation, label] of Object.entries(labels)) {
    const normalized = context.normalizeCodingStep(step({ tool: "blender.execute", label: "Blender",
      inputJson: JSON.stringify({ operation, path: "station/scene_v002.blend" }) }));
    const timeline = render(answer(), [normalized]);
    assert.equal(timeline.querySelector(".coding-step__name").textContent, label);
    assert.equal(timeline.querySelector(".coding-step__name").title, "blender.execute");
  }
  for (const inputJson of [undefined, "malformed historic input", '{"operation":"unknown"}']) {
    const normalized = context.normalizeCodingStep(step({ tool: "blender.execute", inputJson }));
    assert.equal(normalized.label, "Blender", "older receipts remain renderable without a known operation");
  }
});

test("Blender render progress survives the final nested execution receipt without hiding geometry findings", () => {
  const { context, render, document } = harness({ codingToolStepsExpanded: true });
  const initial = context.normalizeCodingStep(step({ tool: "blender.execute", status: "running",
    inputJson: JSON.stringify({ operation: "render", path: "station/scene_v002.blend",
      outputDirectory: "station/review-v002", views: ["perspective", "front", "top"], resolution: 768 }),
    outputJson: JSON.stringify({ stdout: "Rendering perspective 1/3", stderr: "", partial: true }) }));
  const current = render(answer({ status: "streaming" }), [initial]);
  document.body.append(current);
  const disclosure = current.querySelector("details");
  assert.equal(current.querySelector(".coding-output--terminal pre").textContent, "Rendering perspective 1/3");
  const completed = { ...initial, status: "completed", outputJson: JSON.stringify({
    success: true, valid: false, reportPath: "station/review-v002/report.json", counts: { meshes: 42 },
    issues: [{ code: "mesh.degenerate", object: "RoverWheel", message: "Degenerate face" }],
    images: [{ view: "top", path: "station/review-v002/top.png" }],
    execution: { success: true, exitCode: 0, elapsedMilliseconds: 42100,
      stdout: "Rendering perspective 1/3\nSaved top.png 3/3", stderr: "Geometry needs review" }
  }) };
  ...
  assert.equal(current.querySelector(".coding-output--terminal pre").textContent,
    "Rendering perspective 1/3\nSaved top.png 3/3");
  assert.equal(current.querySelector(".coding-output--stderr pre").textContent, "Geometry needs review");
  assert.equal(current.querySelector(".coding-step__status").textContent, "Abgeschlossen");
  for (const fact of ["Exitcode", "report.json", "top.png", "Befunde prüfen", "RoverWheel"])
    assert.ok(current.textContent.includes(fact), `visible Blender receipt includes ${fact}`);
  assert.equal(current.textContent.includes("Keine strukturellen Fehler erkannt"), false,
    "a successful render process cannot turn failed geometry checks into an approval");
});

test("Blender failures show the broker's actionable message in collapsed and expanded receipts", () => {
  const { context, render } = harness();
  const failures = [
    "Blender wurde nicht gefunden. Installiere Blender oder setze GO_BLENDER_EXECUTABLE.",
    "Blender-Eingabedatei wurde seit dem Lesen verändert; aktuellen SHA-256 erneut ermitteln.",
    "Das Ziel existiert bereits; für jede Revision/Prüfung einen neuen Ordner verwenden."
  ];
  for (const message of failures) {
    const failed = context.normalizeCodingStep(step({ tool: "blender.execute", status: "failed",
      inputJson: JSON.stringify({ operation: "render", path: "scene.blend" }),
      outputJson: JSON.stringify({ failed: true, errorCode: "client.tool_failed", message }) }));
    const collapsed = render(answer({ status: "failed" }), [failed]);
    assert.ok(collapsed.querySelector(".coding-step__summary").textContent.includes(message));
    const expanded = render(answer({ status: "failed" }), [failed], { codingToolStepsExpanded: true });
    assert.ok(expanded.querySelector(".coding-step__detail").textContent.includes(message));
    assert.equal(expanded.querySelector(".coding-step__status").textContent, "Fehlgeschlagen");
  }
});

test("Blender info exposes unavailable installations and exact scene metadata", () => {
  const { context, render } = harness({ codingToolStepsExpanded: true });
  const info = output => render(answer(), [context.normalizeCodingStep(step({ tool: "blender.execute",
    inputJson: JSON.stringify({ operation: "info" }), outputJson: JSON.stringify(output) }))]);
  const value = (timeline, label) => timeline.querySelectorAll(".coding-fact")
    .find(fact => fact.querySelector("dt").textContent === label)?.querySelector("dd").textContent;
  const unavailable = info({ available: false, executable: null, file: null });
  assert.equal(value(unavailable, "Verfügbar"), "Nein",
    "a completed availability query must not conceal the missing installation");
  const executable = "C:\\Blender\\blender.exe";
  const sha256 = "a".repeat(64);
  const available = info({ available: true, executable, file: { path: "scene.blend", bytes: 987654, sha256 } });
  assert.equal(value(available, "Verfügbar"), "Ja");
  assert.equal(value(available, "Anwendung"), executable);
  assert.equal(value(available, "Datei"), "scene.blend");
  assert.equal(value(available, "Dateiprüfsumme"), sha256);
  assert.equal(value(available, "Dateigröße"), "987654");
});

test("an earlier Blender stderr warning cannot hide current render progress in a collapsed header", () => {
  const { context, render, document } = harness();
  const warning = "Optional texture warning";
  const initial = context.normalizeCodingStep(step({ tool: "blender.execute", status: "running",
    inputJson: JSON.stringify({ operation: "render", path: "scene.blend" }),
    outputJson: JSON.stringify({ stdout: "Rendering perspective 1/3", stderr: warning, partial: true }) }));
  const current = render(answer({ status: "streaming" }), [initial]);
  document.body.append(current);
  const summary = current.querySelector(".coding-step__summary");
  assert.ok(summary.textContent.includes("Rendering perspective 1/3"));
  assert.ok(summary.textContent.includes(warning));
  const completed = { ...initial, status: "completed", outputJson: JSON.stringify({ success: true,
    execution: { exitCode: 0, stdout: "Rendering perspective 1/3\nSaved top.png 3/3", stderr: warning } }) };
  context.goCodingTimeline.reconcile(current, render(answer(), [completed], { previousTimeline: current }));
  assert.equal(current.querySelector(".coding-step__summary"), summary);
  assert.ok(summary.textContent.includes("Saved top.png 3/3"));
  assert.ok(summary.textContent.includes(warning));
  ...
  assert.equal(current.querySelector(".coding-step__content"), null,
    "progress stays visible without forcing users to expand a large receipt");
});
